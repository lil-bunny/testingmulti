"""
Remove rate-confirmation pages from mixed POD PDFs.

Live POD graph path
-------------------
After ``pod_analysis``, ``trim_local_pdf`` reads LLM ``pages[]``
(``page_type == RATE_CONFIRMATION``), rebuilds the merged local PDF without
those pages, and writes ``pod_*_trimmed.pdf`` when any page remains.

Shared primitive
----------------
``rebuild_without_pages`` is the only pikepdf rebuild path — used by the live
LLM trim and by the legacy OCR helpers below.

Legacy OCR helpers (not on the live POD graph)
----------------------------------------------
``strip_pdf_bytes`` / ``find_rate_confirmation_pages`` / ``resolve_ratecon_page_count``
remain available for offline reuse. Prefer LLM page types for production POD.
"""

from __future__ import annotations

import io
from dataclasses import dataclass, field
from pathlib import Path
from typing import Any, Literal

from app.core.db import db_scope
from app.core.logger import get_logger
from app.domain.pod_lifecycle.rate_confirmation_heading import (
    page_has_rate_confirmation_heading,
)
from app.models.document_analysis import (
    DOCUMENT_ANALYSIS_PAGE_COUNT_KEY,
    DocumentAnalysisType,
)
from app.tools.documents import _uuid_or_none
from app.tools.pdf_page_text_extractor import (
    PdfPageTextExtractor,
    pdf_page_count,
)

logger = get_logger(__name__)

RATE_CONFIRMATION_PAGE_TYPE = "RATE_CONFIRMATION"

TrimOutcome = Literal["continue", "only_ratecon"]


# ---------------------------------------------------------------------------
# Results
# ---------------------------------------------------------------------------


@dataclass(frozen=True)
class RateconPageRebuildResult:
    """Kept PDF bytes after pages are removed (or a skip reason)."""

    kept_pdf_bytes: bytes | None
    excluded_page_numbers: list[int] = field(default_factory=list)
    kept_page_count: int = 0
    original_page_count: int = 0
    skip_reason: str | None = None

    @property
    def success(self) -> bool:
        return self.kept_pdf_bytes is not None and self.kept_page_count > 0


@dataclass(frozen=True)
class TrimLocalPdfResult:
    """Outcome of LLM-driven ratecon page trim on a local merged PDF."""

    outcome: TrimOutcome
    trimmed_local_path: str | None = None
    excluded_page_numbers: list[int] = field(default_factory=list)
    original_page_count: int = 0
    kept_page_count: int = 0
    error: str | None = None

    @property
    def success(self) -> bool:
        return self.error is None


# ---------------------------------------------------------------------------
# Pure helpers (LLM pages + cache hint)
# ---------------------------------------------------------------------------


def collect_ratecon_page_numbers(pages: Any) -> tuple[list[int] | None, str | None]:
    """
    Return 1-based RATE_CONFIRMATION page numbers from LLM ``pages[]``.

    ``(None, error)`` when ``pages`` is missing/malformed and trim cannot proceed.
    """
    if not isinstance(pages, list) or not pages:
        return None, "pages_unusable"
    excluded: list[int] = []
    for page in pages:
        if not isinstance(page, dict):
            return None, "pages_unusable"
        page_number = page.get("page_number")
        if not isinstance(page_number, int) or page_number < 1:
            return None, "pages_unusable"
        page_type = str(page.get("page_type") or "").strip().upper()
        if page_type == RATE_CONFIRMATION_PAGE_TYPE:
            excluded.append(page_number)
    return sorted(set(excluded)), None


def _page_count_from_document_analysis_row(row: dict[str, Any] | None) -> int | None:
    if not row:
        return None
    metadata = row.get("metadata")
    if not isinstance(metadata, dict):
        return None
    raw = metadata.get(DOCUMENT_ANALYSIS_PAGE_COUNT_KEY)
    try:
        value = int(raw)
    except (TypeError, ValueError):
        return None
    return value if value >= 1 else None


def _read_cached_ratecon_extraction_row(shipments_row_id: str | None) -> dict[str, Any]:
    """Load cached ``ratecon_extraction`` row for ``shipments_row_id``. Returns ``{found, row?, error?}``."""
    row_shipment_id = _uuid_or_none(shipments_row_id)
    if shipments_row_id and not row_shipment_id:
        return {"found": False, "error": "invalid_shipments_row_id"}
    if not row_shipment_id:
        return {"found": False, "error": "missing_shipments_row_id"}

    try:
        with db_scope() as repos:
            row = repos.document_analysis.get_by_shipment_and_type(
                shipment_id=row_shipment_id,
                analysis_type=DocumentAnalysisType.RATECON_EXTRACTION.value,
            )
        if not row:
            return {"found": False}
        if row.get("document_id") is not None:
            row["document_id"] = str(row["document_id"])
        return {"found": True, "row": row}
    except Exception as exc:
        logger.exception(
            "ratecon_page_trim: read cached ratecon_extraction failed shipment_id=%s",
            row_shipment_id,
        )
        return {"found": False, "error": str(exc)}


def resolve_ratecon_page_count(shipments_row_id: str | None) -> int | None:
    """
    Cached ratecon PDF page count (R hint for the legacy terminal-window strip), or ``None``.

    ``None`` means "no hint available" — callers fall back to match-only OCR strip.
    Not used by the live LLM trim path.
    """
    out = _read_cached_ratecon_extraction_row(shipments_row_id)
    if not out.get("found"):
        return None
    return _page_count_from_document_analysis_row(out.get("row"))


# ---------------------------------------------------------------------------
# Service
# ---------------------------------------------------------------------------


class RateconPageTrimService:
    """
    Rate-confirmation page removal for POD PDFs.

    Primary (live graph): ``trim_local_pdf`` — LLM ``pages[]`` → local trimmed PDF.
    Shared: ``rebuild_without_pages``.
    Legacy OCR: ``strip_pdf_bytes`` / ``find_rate_confirmation_pages`` (unused on live POD).
    """

    def __init__(
        self,
        page_text_extractor: PdfPageTextExtractor | None = None,
    ) -> None:
        self._page_text_extractor = page_text_extractor or PdfPageTextExtractor()

    # ---- Live POD path -----------------------------------------------------

    def trim_local_pdf(
        self,
        *,
        merged_local_path: str,
        pages: Any,
        stage_dir: str | None = None,
        shipment_number: str | None = None,
    ) -> TrimLocalPdfResult:
        """
        Drop LLM-labeled RATE_CONFIRMATION pages from a local merged PDF.

        Outcomes: ``continue`` (trimmed path set when pages removed), ``only_ratecon``
        (zero pages remain), or ``error`` set for hard-fail cases.
        """
        excluded, pages_error = collect_ratecon_page_numbers(pages)
        if pages_error:
            return TrimLocalPdfResult(
                outcome="continue",
                error=pages_error,
            )

        path = Path(str(merged_local_path or "").strip())
        if not path.is_file():
            return TrimLocalPdfResult(
                outcome="continue",
                error="missing_merged_local_path",
            )

        try:
            pdf_bytes = path.read_bytes()
        except OSError as exc:
            logger.warning(
                "ratecon_page_trim: read failed path=%s err=%s",
                path,
                exc,
            )
            return TrimLocalPdfResult(
                outcome="continue",
                error="merged_pdf_read_failed",
            )

        original_count = pdf_page_count(pdf_bytes)
        if original_count < 1:
            return TrimLocalPdfResult(
                outcome="continue",
                error="empty_pdf",
                original_page_count=0,
            )

        assert excluded is not None
        if not excluded:
            return TrimLocalPdfResult(
                outcome="continue",
                trimmed_local_path=None,
                excluded_page_numbers=[],
                original_page_count=original_count,
                kept_page_count=original_count,
            )

        ship = (shipment_number or "unknown").strip() or "unknown"
        rebuilt = self.rebuild_without_pages(
            pdf_bytes,
            excluded=excluded,
            original_count=original_count,
            doc_label=f"pod_trim_{ship}",
        )
        if rebuilt.skip_reason == "all_pages_rate_confirmation" or rebuilt.kept_page_count < 1:
            return TrimLocalPdfResult(
                outcome="only_ratecon",
                trimmed_local_path=None,
                excluded_page_numbers=list(rebuilt.excluded_page_numbers),
                original_page_count=original_count,
                kept_page_count=0,
            )
        if rebuilt.kept_pdf_bytes is None:
            return TrimLocalPdfResult(
                outcome="continue",
                error=rebuilt.skip_reason or "trim_rebuild_failed",
                excluded_page_numbers=list(rebuilt.excluded_page_numbers),
                original_page_count=original_count,
            )

        out_dir = Path(str(stage_dir or "").strip()) if stage_dir else path.parent
        out_path = out_dir / f"pod_{ship}_trimmed.pdf"
        try:
            out_dir.mkdir(parents=True, exist_ok=True)
            out_path.write_bytes(rebuilt.kept_pdf_bytes)
        except OSError as exc:
            logger.warning(
                "ratecon_page_trim: write failed path=%s err=%s",
                out_path,
                exc,
            )
            return TrimLocalPdfResult(
                outcome="continue",
                error="trimmed_pdf_write_failed",
                excluded_page_numbers=list(rebuilt.excluded_page_numbers),
                original_page_count=original_count,
            )

        logger.info(
            "ratecon_page_trim: trimmed path=%s excluded=%s kept=%s",
            out_path,
            rebuilt.excluded_page_numbers,
            rebuilt.kept_page_count,
        )
        return TrimLocalPdfResult(
            outcome="continue",
            trimmed_local_path=str(out_path),
            excluded_page_numbers=list(rebuilt.excluded_page_numbers),
            original_page_count=original_count,
            kept_page_count=rebuilt.kept_page_count,
        )

    # ---- Shared rebuild ----------------------------------------------------

    def rebuild_without_pages(
        self,
        pdf_bytes: bytes,
        *,
        excluded: list[int],
        original_count: int,
        doc_label: str = "doc",
    ) -> RateconPageRebuildResult:
        """Rebuild ``pdf_bytes`` without the given 1-based page numbers."""
        import pikepdf

        excluded_set = set(excluded)
        if not excluded_set:
            return RateconPageRebuildResult(
                kept_pdf_bytes=pdf_bytes,
                excluded_page_numbers=[],
                kept_page_count=original_count,
                original_page_count=original_count,
            )

        with pikepdf.open(io.BytesIO(pdf_bytes)) as src:
            # Delete highest indexes first so remaining indexes stay valid.
            for page_num in sorted(excluded_set, reverse=True):
                idx = page_num - 1
                if 0 <= idx < len(src.pages):
                    del src.pages[idx]

            kept_count = len(src.pages)
            if kept_count < 1:
                logger.info(
                    "ratecon_page_trim: all pages excluded doc=%s pages=%s excluded=%s",
                    doc_label,
                    original_count,
                    sorted(excluded_set),
                )
                return RateconPageRebuildResult(
                    kept_pdf_bytes=None,
                    excluded_page_numbers=sorted(excluded_set),
                    kept_page_count=0,
                    original_page_count=original_count,
                    skip_reason="all_pages_rate_confirmation",
                )

            buf = io.BytesIO()
            src.save(buf)
            kept_bytes = buf.getvalue()

        logger.info(
            "ratecon_page_trim: rebuilt doc=%s original=%s excluded=%s kept=%s",
            doc_label,
            original_count,
            sorted(excluded_set),
            kept_count,
        )
        return RateconPageRebuildResult(
            kept_pdf_bytes=kept_bytes,
            excluded_page_numbers=sorted(excluded_set),
            kept_page_count=kept_count,
            original_page_count=original_count,
        )

    # ---- Legacy OCR (unused on live POD graph) -----------------------------

    def find_rate_confirmation_pages(
        self,
        pdf_bytes: bytes,
        *,
        prefer_native: bool = False,
        doc_label: str = "doc",
    ) -> list[int]:
        """
        Return 1-based page numbers whose text matches the rate confirmation heading.

        Defaults to header-band OCR because mixed POD packs are often image-only.
        """
        pages = self._page_text_extractor.extract_pages(
            pdf_bytes,
            prefer_native=prefer_native,
            ocr_if_sparse=True,
            header_only_ocr=True,
            doc_label=doc_label,
        )
        return [
            page.page_number
            for page in pages
            if page_has_rate_confirmation_heading(page.text)
        ]

    def strip_pdf_bytes(
        self,
        pdf_bytes: bytes,
        *,
        doc_label: str = "doc",
        ratecon_page_count: int | None = None,
    ) -> RateconPageRebuildResult:
        """
        Rebuild ``pdf_bytes`` without rate confirmation pages (OCR / heading path).

        Outcomes: unchanged bytes (no hits), filtered bytes, or skip_reason
        (``empty_pdf`` / ``all_pages_rate_confirmation``).
        """
        if not pdf_bytes:
            return RateconPageRebuildResult(
                kept_pdf_bytes=None,
                skip_reason="empty_pdf",
            )

        page_count = pdf_page_count(pdf_bytes)
        if page_count < 1:
            return RateconPageRebuildResult(
                kept_pdf_bytes=None,
                skip_reason="empty_pdf",
            )

        r = self._normalize_ratecon_page_count(ratecon_page_count)
        if r is None or page_count < r:
            excluded = self.find_rate_confirmation_pages(
                pdf_bytes,
                prefer_native=False,
                doc_label=doc_label,
            )
            logger.info(
                "ratecon_page_trim: match_only doc=%s P=%s R=%s hits=%s",
                doc_label,
                page_count,
                r,
                excluded,
            )
        else:
            excluded = self._find_pages_terminal_window(
                pdf_bytes,
                page_count=page_count,
                ratecon_page_count=r,
                doc_label=doc_label,
            )
            logger.info(
                "ratecon_page_trim: terminal doc=%s P=%s R=%s excluded=%s",
                doc_label,
                page_count,
                r,
                excluded,
            )

        return self.rebuild_without_pages(
            pdf_bytes,
            excluded=excluded,
            original_count=page_count,
            doc_label=doc_label,
        )

    @staticmethod
    def _normalize_ratecon_page_count(value: int | None) -> int | None:
        if value is None:
            return None
        try:
            n = int(value)
        except (TypeError, ValueError):
            return None
        return n if n >= 1 else None

    def _ocr_page_matches(
        self,
        pdf_bytes: bytes,
        page_numbers: list[int],
        *,
        doc_label: str,
        match_cache: dict[int, bool],
    ) -> dict[int, bool]:
        """OCR missing pages; update and return match_cache for requested pages."""
        need = [n for n in page_numbers if n not in match_cache]
        if need:
            pages = self._page_text_extractor.ocr_pages(
                pdf_bytes,
                page_numbers=need,
                header_only=True,
                doc_label=doc_label,
            )
            for page in pages:
                match_cache[page.page_number] = page_has_rate_confirmation_heading(
                    page.text
                )
        return {n: bool(match_cache.get(n)) for n in page_numbers}

    def _find_pages_terminal_window(
        self,
        pdf_bytes: bytes,
        *,
        page_count: int,
        ratecon_page_count: int,
        doc_label: str,
    ) -> list[int]:
        """
        OCR page 1 and page P; strip an R-page window from a matching terminal.

        Front hit → ``1..R`` (wins if both terminals match). Back hit →
        ``P-R+1..P``. Far-end heading miss is logged; window still stripped.
        Neither terminal → match-only full scan.
        """
        r = ratecon_page_count
        p = page_count
        match_cache: dict[int, bool] = {}
        terminals = [1] if p == 1 else [1, p]
        matches = self._ocr_page_matches(
            pdf_bytes,
            terminals,
            doc_label=doc_label,
            match_cache=match_cache,
        )
        front_hit = bool(matches.get(1))
        back_hit = bool(matches.get(p)) if p > 1 else front_hit

        if front_hit and back_hit and p > 1:
            logger.info(
                "ratecon_page_trim: both_terminals_match doc=%s P=%s R=%s "
                "choosing_front",
                doc_label,
                p,
                r,
            )

        if front_hit:
            start, end = 1, r
            far = end
            if far != 1:
                far_matches = self._ocr_page_matches(
                    pdf_bytes,
                    [far],
                    doc_label=doc_label,
                    match_cache=match_cache,
                )
                far_ok = bool(far_matches.get(far))
            else:
                far_ok = True
            if not far_ok:
                logger.warning(
                    "ratecon_page_trim: terminal_far_miss doc=%s anchor=front "
                    "window=%s..%s far_page=%s still_stripping",
                    doc_label,
                    start,
                    end,
                    far,
                )
            else:
                logger.info(
                    "ratecon_page_trim: terminal_front doc=%s window=%s..%s "
                    "far_match=%s",
                    doc_label,
                    start,
                    end,
                    far_ok,
                )
            return list(range(start, end + 1))

        if back_hit:
            start, end = p - r + 1, p
            far = start
            if far != p:
                far_matches = self._ocr_page_matches(
                    pdf_bytes,
                    [far],
                    doc_label=doc_label,
                    match_cache=match_cache,
                )
                far_ok = bool(far_matches.get(far))
            else:
                far_ok = True
            if not far_ok:
                logger.warning(
                    "ratecon_page_trim: terminal_far_miss doc=%s anchor=back "
                    "window=%s..%s far_page=%s still_stripping",
                    doc_label,
                    start,
                    end,
                    far,
                )
            else:
                logger.info(
                    "ratecon_page_trim: terminal_back doc=%s window=%s..%s "
                    "far_match=%s",
                    doc_label,
                    start,
                    end,
                    far_ok,
                )
            return list(range(start, end + 1))

        logger.info(
            "ratecon_page_trim: neither_terminal doc=%s P=%s R=%s → match_only",
            doc_label,
            p,
            r,
        )
        return self.find_rate_confirmation_pages(
            pdf_bytes,
            prefer_native=False,
            doc_label=doc_label,
        )
