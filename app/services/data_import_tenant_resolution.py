...
def resolve_email_data_import_tenant_id(
    *,
    ...
    """
    Match ``payload["webhook_name"]`` to ``tenants.config.email_webhook_name``.
    ...
    names that do not match any ``tenants.config.email_webhook_name`` log a warning.
    """
    ...
    if webhook_name.strip():
        logger.warning(
            "email webhook data_import tenant: no tenants row for config.email_webhook_name=%r",
            webhook_name.strip(),
        )
    return None
