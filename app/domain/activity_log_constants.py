...
STATUS_CHANGE_DESCRIPTION_TEMPLATE = "Status changed from {from_status} to {to_status}"
SUB_STATUS_CHANGE_DESCRIPTION_TEMPLATE = "Sub-status changed from {from_sub_status} to {to_sub_status}"
...
CARRIER_ACK_LLM_ACTION_TEMPLATE = "Carrier ack LLM classified reply as {decision}{confidence_suffix}: {reason}"
...
RATECON_SUPERSEDED_ACTION = "Ratecon cancelled — superseded by new inbound ratecon email"
...
POD_EXTRACTION_PROCESSED_TEMPLATE = "POD document processed — LLM extraction{confidence_suffix}"
POD_ONLY_RATECON_PAGES_FOUND_INFO = "Email attachments had no POD pages left after removing rate confirmation"
