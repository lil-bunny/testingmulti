...
def test_pod_analysis_local_only_without_s3_key(
    mock_resolve: MagicMock,
    mock_bucket: MagicMock,
    mock_extract: MagicMock,
    tmp_path,
) -> None:
    """Post-merge / pre-upload: local path is enough; S3 key not required."""
    ...
    assert out["pod_object_key"] is None
    assert out["findings"]["metadata"]["pod_bytes_source"] == "local_stage"
    assert out["findings"]["metadata"]["pod_object_key_source"] is None
...
@patch("app.tools.pod.bucket")
@patch("app.tools.pod.resolve_merged_pod_object_key")
def test_pod_analysis_skips_when_no_local_and_no_s3(
    mock_resolve: MagicMock,
    mock_bucket: MagicMock,
    mock_extract: MagicMock,
) -> None:
    ...
    out = pod_analysis({"shipment_id": "SHIP"})

    mock_bucket.download_object_bytes.assert_not_called()
    mock_extract.assert_not_called()
    assert out["success"] is True
    assert out["skipped"] is True
    assert out["reason"] == "no_pod_source"
