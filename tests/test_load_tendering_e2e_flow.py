...
def _load_type_branch(to_email: str) -> dict[str, Any]:
    return {
        "send_tender_email": {
            "emails": {"to": [to_email], "cc": [], "bcc": []},
            "email_subject": "Tender # {order_number}",
            "email_template_html": "<p>{reason_for_failure}Tender for order {order_number}</p>",
        },
...
def _tenant_settings_json(*, carrier_email: str = "carrier@example-test.com") -> dict[str, Any]:
    return {
        ...
        "load_tendering": {
            "ltl": _load_type_branch(carrier_email),
            "ftl": {**_load_type_branch(carrier_email), "max_attempts": 3},
            "reminders": {
                "variants": {
                    "ftl": [
    ...
    with psycopg.connect(dsn) as conn:
        ...
        with conn.cursor() as cur:
            cur.execute(
                ...
                (tenant_id, "E2E Load Tendering Test", slug, json.dumps(_tenant_settings_json())),
            )
    try:
...
def _fetch_lifecycle(lifecycle_id: str) -> dict | None:
    with psycopg.connect(_dsn()) as conn:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT id, status, sub_status, tender_id FROM workflow_lifecycles WHERE id = %s",
                (lifecycle_id,),
            )
            row = cur.fetchone()
            if row is None:
