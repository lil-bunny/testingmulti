...
def test_api_load_tendering_rows_404(monkeypatch) -> None:
    from fastapi.testclient import TestClient

    from app.main import create_app

    def _no_rows(self, *args, **kwargs):
        return None, {}

    monkeypatch.setattr(
        DataImportsReadService,
        "get_projected_rows",
        _no_rows,
    )
    client = TestClient(create_app())
    r = client.get(
        "/api/data-imports/bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb/load-tendering-rows",
        params={"tenant_id": "aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa"},
    )
    assert r.status_code == 404


def test_api_load_tendering_rows_200(monkeypatch) -> None:
    from fastapi.testclient import TestClient

    from app.main import create_app

    def _ok(self, *args, **kwargs):
        sample = _sample_raw_data()
        base = list(iter_spreadsheet_rows(sample))
        rows = [project_row(r, LOAD_TENDERING_ROW_PROJECTION) for r in base]
        return rows, {"source": "spreadsheet"}
    ...
    assert body["rows"][0]["customer_match"] == "Acme"
